...
st.title('Task 13 - Wind turbine normal shutdown simulation')
...
with st.expander("**Objective**",True):
	...
	st.write('''<div style="text-align: justify">
			...
			To model a normal shutdown process you should start the simulation in the same setup as for the normal operation cases and use the manual pitch maneuvers to feather out the blades.
...
5. Modify the **ServoDyn input file**:
	...
	- Define the pitch value compatible with parked/idling conditions (for instance, 20 deg)
	- Define when to stop the generator
			</div>''',unsafe_allow_html=True)
	#checkfile = st.checkbox('**Show ElastoDyn file details**')
	if checkfile:
		data = []
...
with st.expander("**Data analysis**",True):
	...
	file = []
	file.append(st.file_uploader("Normal shutdown simulation",accept_multiple_files=False))
...
if export_as_pdf:
	try:
		create_pdf_task13(fig,report_text,'Task 13: Wind turbine normal shutdown simulation','Task13_report',exp_c[1],exp)
	except:
