...
# -- Set page config
apptitle = 'OpenFAST Course - Task 11'
...
with st.expander("**Objective**",True):
	...
	st.write('''<div style="text-align: justify">
			\nAlthogh a wind turbine may be fully parked for some environmental conditions or maintenance operations, for low wind speeds wind turbines are typically allowed to rotate 
			even if no power production is taking place, in what is known as idling. Simulate the response of an idling wind tubrine.
...
2. Modify the **ElastoDyn input file**:
	- Enable the generator degree of freedom (GenDOF) and enable the drivetrain flexibility (DrTrDOF) in the <b>DEGREES OF FREEDOM</b>
	- Set the rotor angular velocity to a low value and the blades' pitch angle to 90 degrees in the <b>INITIAL CONDITIONS</b> section.
	- Define the generator inertia and reduce the gearbox efficieny for a value below 100%
...
3. Modify the **AeroDyn input file**:
	- Since you are making an idling simulation, disable the indcution wake model (WakeMod) 
...
5. Modify the **ServoDyn input file**:
	- Define the generator turn on time such that it is not used during the full simulation (> simulation time)	
			</div>''',unsafe_allow_html=True)
	#checkfile = st.checkbox('**Show ElastoDyn file details**')
	if checkfile:
		data = []
...
with st.expander("**Data analysis**",True):
	...
	file = []
	file.append(st.file_uploader("Idling simulation",accept_multiple_files=False))
...
if export_as_pdf:
	try:
		create_pdf_task11(fig,report_text,'Task 11: Idling wind turbine simulation','Task11_report',exp_c[1],exp)
	except:
