...
st.title('Task 14 - Wind turbine emergency shutdown simulation')
...
with st.expander("**Objective**",True):
	...
	st.write('''<div style="text-align: justify">
			\nWhen the wind speeds are not within the operation range of a given wind turbine, power production should be stopped.
			In most situations, this is accomplished by using the blade pitch control to minimise the aerodynamic torque on the rotor, a process usually refered to as feathering out the blades.
			However, in some extrem cases, for instance in energy loss cases, emergency breaks must be used to stop the rotor.
			To model an emergency shutdown process you should start the simulation in the same setup as for the normal operation cases and use the manual pitch maneuvers to feather out the blades.
...
3. Modify the **AeroDyn input file**:
	- Since you are making a simulation wihtout power production for most of the simulation time, disable the induction wake model (WakeMod) 
...
5. Modify the **ServoDyn input file**:
	- Enable the automatic pitch control (PCMode = 3)
	- Define when the grid loss occurs (for instance, 600s)
	- Define the pitch maneuvers rate (for instance, 2deg/s)
	- Define the pitch value compatible with parked conditions (for instance, 90 deg)
	- Define the time instant to start enabling the HHS break (for instance, 601s), the maximum resistant torque (6000 kN.m) and how long it takes to fully deploy it (2s)
			</div>''',unsafe_allow_html=True)
	#checkfile = st.checkbox('**Show ElastoDyn file details**')
	if checkfile:
		data = []
		...
		tab1,tab2,tab3,tab4 = st.tabs(['**Pitch control**',
									  '**Generator and torque control**',
									  'Simple variable-speed torque control',
									  '**HIgh-speed shaft break**'])
...
with st.expander("**Data analysis**",True):
	...
	file = []
	file.append(st.file_uploader("Emergency shutdown simulation",accept_multiple_files=False))
...
if export_as_pdf:
	try:
		create_pdf_task14(fig,report_text,'Task 14: Wind turbine emergency shutdown simulation','Task14_report',exp_c[1],exp)
	except:
