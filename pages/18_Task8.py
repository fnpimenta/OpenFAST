...
# -- Set page config
apptitle = 'OpenFAST Course - Task 8'
icon = Image.open('feup_logo.ico')
st.set_page_config(page_title=apptitle, page_icon=icon)

st.title('Task 8 - Normal operation simulations with different torque control parameters')
...
with st.expander("**Objective**",True):
	...
	st.write('''<div style="text-align: justify">
			\nTo study the impact of the torque control curve parameters on the response, 
			repeat Task 7 but with a higher and lower variable speed generator curve. 
			For clarity, it is suggested that you use a low wind speed (for instance, 6m/s), ensuring that the wind turbine is operating in Region 2, where the torque control impact is more visible.
...
2. Modify the **ServoDyn input file**:
	- Modify the torque control curve parameters in the <b>SIMPLE VARIABLE-SPEED TORQUE CONTROL</b> section such that, for the same generator angular velocity:
  		- The generator torque is 1.25 higher than the reference curve (Simulation 1)                    
  		- The generator torque is 1.25 lower than the reference curve (Simulation 2)
			</div>''',unsafe_allow_html=True)
...
4. Modify the **InflowWind input file** (or use the same from Task 6):
	...
	- Include the paths to the corresponding TurbSim generated (.bts) with a low wind speed (for instance, 6 m/s) file in <b>Parameters for Binary TurbSim Full-Field files</b> section
...
with st.expander("**Data analysis**",True):
	...
	cols0 = st.columns(3)
	file = []
	file.append(cols0[0].file_uploader("Reference curve (Task 7)",accept_multiple_files=False))
	file.append(cols0[1].file_uploader("Amplified curve",accept_multiple_files=False))
...
if export_as_pdf:
	try:
		create_pdf_task8(fig,report_text,'Task 8: Torque curve parameters','Task8_report',exp_c[1],exp)
	except:
