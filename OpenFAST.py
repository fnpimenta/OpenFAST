...
class OpenFAST:
    ...
    def __init__(self,FileName,tmin=0,label=''):
        ...
        #    st.write('Automatically loaded OpenFAST simulation results.')
        except:
            st.write('No data file available.')
    ...
    def get_results(self):
        self.data = pd.read_csv(self.name , skiprows=[0,1,2,3,4,5,7] , delimiter=r"\s+",header=0)
        self.name.seek(0)
        self.units = pd.read_csv(self.name, skiprows=[0,1,2,3,4,5,6] , nrows=0,delimiter=r"\s+",header=0).columns
